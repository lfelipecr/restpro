...
var obtenerDatos = function(){
    $.ajax({
        ...
        success: function(item){
            ...
            $('#hidden_amazonas').val(item.ose);
            $('#certpse_hidden').val(item.certpse);
            if(item.sunat == '1'){$('#sunat').prop('checked', true)};
            if(item.modo == '1'){$('#modo').prop('checked', true)}; 
            if(item.certpse == '1'){$('#certpse').prop('checked', true)};
            if(item.ose == '1'){$('#ose').prop('checked', true); $('.ose_url').css('display','block');};     
            if(item.amazonas == '1'){$('#amazonas').prop('checked', true);};      
        }
    });
}
...
$('#amazonas').on('click', function(event){
    if($('#amazonas').is(':checked')){
        $('#hidden_amazonas').val('1');
    }else{
        $('#hidden_amazonas').val('0');
    }
});

$('#certpse').on('change', function(event){
    if($(this).prop('checked')){
        $('#certpse_hidden').val('1');
    }else{
        $('#certpse_hidden').val('0');
    }
});
