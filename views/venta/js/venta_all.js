            ...
            $.each(data.Detalle, function(i, item) {
                ...
                if (item.comentario.length > 0) {
                    ...
                    $.each(arreglo1, function (ind, elem) {
                        console.log(elem)
                        if (elem != '') {
                            notas_ += '<span class="label label-info ml-1 mr-1 font-13">'+elem+'</span>';
                        }
                    });
                    var notas_t = 'Notas:';
                }
                ...
                if (item.estado != 'z'){
                    ...
                    .append(
                        ...
                        +'<p class="m-b-0 font-13 mt-1">'+notas_t+' '+notas_+' </p></div>'));
                } else {
                    $('#list-subitems')
                    .append(
                        $('<div class="d-flex flex-row comment-row comment-list"/>')
                        ...
                        +'<p class="m-b-0 font-13 mt-1">'+notas_t+' '+notas_+' </p>'
                        +'</div>'));
                }
            });
        },
...
var anularPedido = function(id_pedido,id_pres,cant_ped,prod_nom,prod_pre,fecha_pedido,estado_pedido){
    ...
    $('body').css('padding-right','0px'); 
    var html_confirm = '<div>Se anulará el siguiente pedido:<br><label class="text-danger font-bold">'+cant_ped+' UNI</label> '+prod_nom+' <span class="label label-warning">'+prod_pre+'</span></label><br><br>Ingrese código de seguridad111</div>\
...
// console.log(data)
            ...
            for(y=0; y < cont.length; y++){
                ...
                var nuevopedido = {
                    ...
                    pedido_numero : $('.pedido-numero').text(),
                    pedido_cliente : $('.pedido-cliente').text(),
                    direccion_cliente : $('.pedido-direccion').text(),
                    pedido_mozo : $('#nombre_mozo').val(),
                    direccion_comanda : $('#direccion_comanda').val(),
                    pedido_comanda : $('#pedido_comanda').val(),
                    correlativo_imp : correlativo_imp(),  
                    nombre_imp : nombre_impresora,
                    nombre_pc : $('#pc_name').val(),
                    host_pc : $('#host_pc').val(),
...
var reimpresion_comanda = function(id_key){
    // console.log('dddd');
    $.ajax({
        //cache: false,
        async: false,
        type: 'POST',
        dataType: 'JSON',
        url: $('#url').val()+'venta/reimpresion_comanda',
        data: {
            id_key: id_key 
        },
        success: function (data) {
            ...
            for(i=0; i < array.length; i++){
                // contador.push(array[i].id_areap); // selecciona las impresoras asi sean duplicadas
                contador.push(parseInt(array[i].id_areap));
            }
            ...
            for(y=0; y < cont.length; y++){
                ...
                var nuevopedido = {
                    ...
                    pedido_cliente : $('.pedido-cliente').text(),
                    pedido_mozo : $('#nombre_mozo').val(),
                    pedido_telefono : $('.pedido-telefono').text(),
                    pedido_direccion : $('.pedido-direccion').text(),
                    correlativo_imp : correlativo_imp(),  
                    nombre_imp : nombre_impresora,
                    nombre_pc : $('#pc_name').val(),
                    host_pc : $('#host_pc').val(),
