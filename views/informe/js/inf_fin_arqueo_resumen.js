...
var defaultdata = function(){
    $.ajax({
        ...
        success: function (item) {
            ...
            var totalIng = (parseFloat(item.total) + parseFloat(item.pagos_plin_mix.total_plin) + parseFloat(item.pagos_yape_mix.total_yape) + parseFloat(item.pagos_tran_mix.total_tran) + parseFloat(item.Ingresos.total)).toFixed(2);
            ...
            var montoEfectivo = (parseFloat(item.Apertura.monto_aper) + (parseFloat(totalIng) - parseFloat(item.pago_tar) - parseFloat(item.pago_yape) - parseFloat(item.pago_plin) - parseFloat(item.pago_tran)) - parseFloat(totalEgr)).toFixed(2);
            ...
            if(montoDiferencia > 0){
                ...
                $.each(item.Pagos, function(i, pago) {
                    if (pago.DESCRIPCION==="EFECTIVO") {
                        EfectivoTotal += pago.MONTO;
                    }
                    if (pago.DESCRIPCION==="TARJETA") {
                        tarjetaTotal += pago.MONTO;
                    }
                    if (pago.DESCRIPCION==="YAPE") {
                        yapeTotal += pago.MONTO;
                    }
                    if (pago.DESCRIPCION==="PLIN") {
                        plinTotal += pago.MONTO;
                    }
                    if (pago.DESCRIPCION==="TRANSFERENCIA") {
                        tranTotal += pago.MONTO;
                    }

                });
            
            $(".c-monto-tarjeta").html(moneda+' '+formatNumber(tarjetaTotal));
            // $(".c-monto-tarjeta").html(moneda+' '+formatNumber(parseFloat(tarjetaTotal) + parseFloat(item.pagos_tar_mix.total_tar) ));
            $(".c-monto-yape").html(moneda+' '+formatNumber(parseFloat(yapeTotal) + parseFloat(item.pagos_yape_mix.total_yape) ));
            $(".c-monto-plin").html(moneda+' '+formatNumber(parseFloat(plinTotal) + parseFloat(item.pagos_plin_mix.total_plin) ));
            $(".c-monto-tran").html(moneda+' '+formatNumber(parseFloat(tranTotal) + parseFloat(item.pagos_tran_mix.total_tran) ));
