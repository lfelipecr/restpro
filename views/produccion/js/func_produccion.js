    		...
    		$('#agrupacion_pedidos_list')
    			.append(
	                ...
	                .html(''
	                	+'<div class="card">'
		                	+'<div class="card-body p-0">'
			                	+'<div class="d-flex flex-wrap p-20 p-b-10 justify-center" style="background: #8196ac;"><div>'
			                		+'<h3 class="font-bold text-white">'+tipoatencion+''+item.nro_mesa+'</h3>'
			                		+'<h4 class="text-white">'+item.desc_salon+'</h4>'
			                	+'</div>'
			                	+'<div class="ml-auto align-self-center">'
			                		+'<a onclick="atendidopormesa('+item.id_pedido+');">'
				                    	+'<button type="button" class="btn btn-circle btn-lg btn-success"><i class="ti-check"></i></button>'
				                    +'</a>'
			                	+'</div>'
			                	+'<div class="ml-auto align-self-center">'
			                		+'<label class="font-bold font-30 m-b-0" style="color: #fff947 !important;">'
			                		+'<span class="total'+i+'"></span> UNI</label>'
			                	+'</div>'
			                +'</div>'
	                	+'<table class="table stylish-table b-t" id="table'+i+'">'
	                		+'<thead class="table-head">'
...
var atendidopormesa = function(cod_ped){
	$.ajax({
		dataType: 'JSON',
		type: 'POST',
		url: $('#url').val()+'produccion/atendidopormesa',
		data: {
      		cod_ped: cod_ped
      	},	
      	success: function (datos) {
