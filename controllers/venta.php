...
class Venta extends Controller {
    ...
    function comensal(){
        print_r(json_encode($this->model->comensal()));
    }
    ...
    function impresion_reparto_pedido($id_pedido)
    {
        if(Session::get('print_cpe') == 1){
            $dato = $this->model->impresion_reparto_pedido($id_pedido);
            header('location: http://'.Session::get('pc_ip').'/imprimir/ticket_reparto.php?data='.json_encode($dato));            
        } else {
            $this->view->dato = $this->model->impresion_reparto_pedido($id_pedido);
            $this->view->render('venta/imprimir/ticketreparto', true);
        }
    }
