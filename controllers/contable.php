<?php 
	Session::init(); 
	$ver = (Session::get('rol') == 1 OR Session::get('rol') == 2 OR Session::get('rol') == 3 OR Session::get('rol') == 7) ? '' :  header('location: ' . URL . 'err/danger'); 
...
class Contable extends Controller {
    ...
    function excel() 
	{
		// echo $_POST['tipoBusqueda_hidden']; 
		// return;
        ...
        if ($_POST['tipoBusqueda_hidden']==1) {
        	$this->view->periodo = 'DEL '.$_POST['start'].' AL '.$_POST['end'].'';
        }else{
        	$this->view->periodo = $_POST['month'];
        }
	...
	function validar_cpe()
    {
        $this->model->validar_cpe($_POST);
    }
}