...
class Informe extends Controller {
	...
	function finanza_arq_imp_prod($id)
    {
        if(Session::get('imp_cierre') == 1){
	        $usuario = array(
			    "nombres" => Session::get('nombres'),
			    "apellidos" => Session::get('apellidos'),
			    // "server" => Session::get('pc_ip'),
			);
	        $dato = $this->model->finanza_arq_imp($id);
            header('location: http://'.Session::get('pc_ip').'/imprimir/arqueo_prod.php?data='.json_encode($dato).'&usuario='.json_encode($usuario).'');            
        } else {
			$this->view->empresa = $this->model->Empresa();
			$this->view->dato    = $this->model->finanza_arq_imp($id);
			$this->view->render('informe/finanza/imprimir/imp_cierre_prod', true);
        }

    }

	function finanza_arq_imp_gastos($id)
    {
        if(Session::get('imp_cierre') == 1){
	        $usuario = array(
			    "nombres" => Session::get('nombres'),
	        ...
	        $dato = $this->model->finanza_arq_gastos($id);
            header('location: http://'.Session::get('pc_ip').'/imprimir/arqueo_gastos.php?data='.json_encode($dato).'&usuario='.json_encode($usuario).'');            
        } else {
			$this->view->empresa = $this->model->Empresa();
			$this->view->dato    = $this->model->finanza_arq_gastos($id);
			$this->view->render('informe/finanza/imprimir/imp_cierre_gastos', true);
        }
