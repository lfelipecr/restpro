    ...
    */
    ...
    public function listarProductos($data)
    {
        ...
        {   
            ...
            $stm = $this->db->prepare("SELECT id_areap,id_pres,pro_cod,precios,pro_nom,pro_pre".$campo.",pro_img FROM v_productos WHERE id_catg = ? AND est_a = 'a'  AND est_b = 'a' AND est_c = 'a' ".$variable);
    ...
    */
    ...
    public function RegistrarVenta($data)
    {
        try 
        {
            ...
            if( ($conteo->conteo > 0) &&  $data['total'] > 0){    
                $consulta = "call usp_restEmitirVenta(:flag, :dividir_cuenta, :id_pedido, :tipo_pedido, :tipo_entrega, :id_cliente, :id_tipo_doc, :id_tipo_pago, :id_usu, :id_apc, :pago_efe_none, :pago_tar, :pago_yape, :pago_plin, :pago_tran, :descuento_tipo, :descuento_personal, :descuento_monto, :descuento_motivo, :comision_tarjeta, :comision_delivery, :igv, :total, :codigo_operacion, :fecha_venta);";
                $arrayParam = array(
                    ':flag' => 1,
                    ...
                    ':pago_yape' => $data['pago_yape'] ? $data['pago_yape'] : 0,
                    ':pago_plin' => $data['pago_plin'] ? $data['pago_plin'] : 0,
                    ':pago_tran' => $data['pago_tran'] ? $data['pago_tran'] : 0,
                ...
                $sqlconsumo = "UPDATE tm_venta SET consumo = ?, consumo_desc = ?, observacion = ?  WHERE id_venta = ? ";
                $this->db_consumo->prepare($sqlconsumo)->execute(array($data['venta_por_consumo'],$data['venta_por_consumo_concepto'],$data['observacion'],$id_venta));
    ...
    public function venta_edit_documento($data)
    {
        try 
        {
            ...
            $this->db_edit->prepare($sqledit)->execute(array(date('Y-m-d h:m:s',strtotime($data['fecha_nueva'])),$data['nv_original'],date('Y-m-d',strtotime($data['nv_fecha'])),$data['id_venta']));
    ...
    { 
        try
        {      
            $stm = $this->db->prepare("SELECT v.id_pedido, v.id_cliente, v.desc_tp, v.id_tpag, v.paga_con, v.comision_delivery, d.nro_pedido FROM v_pedido_delivery AS v INNER JOIN tm_pedido_delivery AS d ON v.id_pedido = d.id_pedido WHERE v.id_pedido = ?");
            $stm->execute(array($id_venta));
            $c = $stm->fetch(PDO::FETCH_OBJ);
