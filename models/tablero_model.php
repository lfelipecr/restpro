	...
	public function tablero_datos()
    {
        try
        {
            ...
            */
            ...
            $ventas->{'Efectivo'} = $this->db->query("SELECT IF(v.id_tpag = 1 OR v.id_tpag = 3,SUM(v.pago_efe),0) AS total, COUNT(*) AS cant FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$id_apc} AND v.estado <> 'i' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 2000) AND (v.id_tpag = 1 OR v.id_tpag = 3)")
            ->fetch(PDO::FETCH_OBJ);

            $ventas->{'Pagos'} = $this->db->query("SELECT b.DESCRIPCION, 
            ...
            WHERE a.id_tipo_pago = b.id_tipo_pago AND a.id_tipo_pago != 3 AND a.id_apc = {$id_apc} AND a.estado <> 'i' GROUP BY a.id_tipo_pago")
            // WHERE a.id_tipo_pago = b.id_tipo_pago AND a.id_tipo_pago != 3 AND a.id_apc = {$data} GROUP BY a.id_tipo_pago")
            ->fetchAll(PDO::FETCH_OBJ);

            $ventas->{'pagos_yape_mix'} = $this->db->query("SELECT IFNULL(SUM(pago_yape),0) as total_yape FROM tm_venta WHERE id_tipo_pago = 3 AND id_apc = {$id_apc} AND estado <> 'i'")->fetch(PDO::FETCH_OBJ);

            $ventas->{'pagos_plin_mix'} = $this->db->query("SELECT IFNULL(SUM(pago_plin),0) as total_plin FROM tm_venta WHERE id_tipo_pago = 3 AND id_apc = {$id_apc} AND estado <> 'i'")->fetch(PDO::FETCH_OBJ);

            $ventas->{'pagos_tran_mix'} = $this->db->query("SELECT IFNULL(SUM(pago_tran),0) as total_tran FROM tm_venta WHERE id_tipo_pago = 3 AND id_apc = {$id_apc} AND estado <> 'i'")->fetch(PDO::FETCH_OBJ);


            $sql_caja = $this->db->prepare("SELECT * FROM v_caja_aper WHERE /*(fec_ven >= ? AND fec_ven < ?) AND*/ id_apc = ? AND estado = 'a'");
            $sql_caja->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $caja= $sql_caja->fetch(PDO::FETCH_OBJ); 
            ...
            $sql_canal_salon = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_yape + pago_pline + pago_tran) total_ventas FROM tm_venta WHERE id_tipo_pedido = 1 AND estado = 'a' AND id_apc = ?");
            $sql_canal_salon->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_salon = $sql_canal_salon->fetch(PDO::FETCH_OBJ);

            $sql_canal_mostrador = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_yape + pago_plin + pago_tran) total_ventas FROM tm_venta WHERE id_tipo_pedido = 2 AND estado = 'a' AND id_apc = ?");
            $sql_canal_mostrador->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_mostrador = $sql_canal_mostrador->fetch(PDO::FETCH_OBJ);

            $sql_canal_delivery = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_yape + pago_plin + pago_tran) total_ventas FROM tm_venta WHERE id_tipo_pedido = 3 AND estado = 'a' AND id_apc = ?");
            $sql_canal_delivery->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_delivery = $sql_canal_delivery->fetch(PDO::FETCH_OBJ);

            $sql_canal_salon_i = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_yape + pago_plin + pago_tran) total_ventas FROM tm_venta WHERE id_tipo_pedido = 1 AND estado = 'i' AND id_apc = ?");
            $sql_canal_salon_i->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_salon_i = $sql_canal_salon_i->fetch(PDO::FETCH_OBJ);

            $sql_canal_mostrador_i = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_yape + pago_plin + pago_tran) total_ventas FROM tm_venta WHERE id_tipo_pedido = 2 AND estado = 'i' AND id_apc = ?");
            $sql_canal_mostrador_i->execute(array(/*$ifecha,$ffecha,*/$id_apc));
            $canal_mostrador_i = $sql_canal_mostrador_i->fetch(PDO::FETCH_OBJ);

            $sql_canal_delivery_i = $this->db->prepare("SELECT COUNT(*) AS cantidad_ventas, SUM(pago_efe + pago_tar + pago_yape + pago_plin + pago_tran) total_ventas FROM tm_venta WHERE id_tipo_pedido = 3 AND estado = 'i' AND id_apc = ?");
            ...
            */
            ...
            $data = array("Caja" => $caja ,"Ventas" => $ventas,"Egresos" => $egresos,"Ingresos" => $ingresos,"Platos" => $platos,"Productos" => $productos, "Pollostock" => $pollos_stock, "Pollosvendidos" => $pollos_vendidos, "CanalSalon" => $canal_salon, "CanalMostrador" => $canal_mostrador, "CanalDelivery" => $canal_delivery, "CanalSalonAnulados" => $canal_salon_i, "CanalMostradorAnulados" => $canal_mostrador_i,"CanalDeliveryAnulados" => $canal_delivery_i/*"data3" => $m,"data4" => $tp,"data5" => $me,"data6" => $vm,"data7" => $vmo,"data8" => $mo,*//*,"data11" => $ma,"data12" => $moa*/);
