...
class Facturacion_Model extends Model
{
	...
	public function Datos1()
    {
        try
        {
            ...
            } else if ($_POST['estado'] == 10) {
                $estado = "AND v.enviado_sunat = '1' AND v.cambiomanual = '0' AND (v.hash_cdr is null or v.hash_cdr = '')";
            } else {
                $estado = "";
            }
            ...
            if ($_POST['input_num'] != '') {
                ...
                $stm = $this->db->prepare("SELECT v.*,IFNULL((v.total+v.comis_del-v.desc_monto),0) AS total FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE nro_doc = ?  AND v.id_tdoc <> 3 GROUP BY v.id_ven");
                $stm->execute(array(str_pad($_POST['input_num'], 8, "0", STR_PAD_LEFT)));
                ...
                $stm = $this->db->prepare("SELECT v.*,IFNULL((v.total+v.comis_del-v.desc_monto),0) AS total FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.id_tdoc like ? AND v.id_cli like ? AND v.id_tdoc <> 3 ".$estado." GROUP BY v.id_ven");
                $stm->execute(array($ifecha,$ffecha,$_POST['tdoc'],$_POST['cliente_id']));

                $c = $stm->fetchAll(PDO::FETCH_OBJ);           
                foreach($c as $k => $d)
                {
                    $c[$k]->{'Cliente'} = $this->db->query("SELECT dni,ruc,nombre,tipo_cliente FROM v_clientes WHERE id_cliente = ".$d->id_cli)
                        ->fetch(PDO::FETCH_OBJ);
                }
                $data = array("data" => $c);
                $json = json_encode($data);
    ...
    public function Datos5()
    {
        ...
        {
            $ifecha = date('Y-m-d',strtotime($_POST['ifecha']));
            $stm = $this->db->prepare("SELECT fec_ven,desc_td,ser_doc,nro_doc,(total-desc_monto) AS total FROM v_ventas_con WHERE id_tdoc = 1 AND enviado_sunat != '1' AND DATE_FORMAT(fec_ven,'%Y-%m-%d') = ?");
    ...
    public function reenvio($data)
    {
        try {
            if ($data['estado'] == null) {
                $stm = $this->db->prepare("UPDATE tm_venta SET enviado_sunat = 1, code_respuesta_sunat = 0, cambiomanual = 1 WHERE id_venta = ?");
                $stm->execute(array($data['cod_ven']));
                $c = $stm->fetchAll(PDO::FETCH_OBJ);
            }else if ($data['estado'] == '11'){
                $stm = $this->db->prepare("UPDATE tm_venta SET enviado_sunat = 1, code_respuesta_sunat = 0, cambiomanual = 1, estado = 'i' WHERE id_venta = ?");
                $stm->execute(array($data['cod_ven']));
                $c = $stm->fetchAll(PDO::FETCH_OBJ);
            }else{
                $stm = $this->db->prepare("UPDATE tm_venta SET enviado_sunat = 0, cambiomanual = 1, hash_cdr = null, name_file_sunat = null, code_respuesta_sunat = null, descripcion_sunat_cdr = null, estado = 'a' WHERE id_venta = ?");
                $stm->execute(array($data['cod_ven']));
                $c = $stm->fetchAll(PDO::FETCH_OBJ);
            }
            return $c;
    ...
    public function cambioFecha($data)
    {
        try {
                $stm = $this->db->prepare("UPDATE tm_venta SET fecha_venta = ? WHERE id_venta = ?");
                $stm->execute(array(''.$data['nuevafecha'].' 09:00:00',$data['cod_ven']));
                $c = $stm->fetchAll(PDO::FETCH_OBJ);
            return $c;
