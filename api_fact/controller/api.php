<?php Session::init(); $ver = (Session::get('rol') == 1 OR Session::get('rol') == 2 OR Session::get('rol') == 3 OR Session::get('rol') == 5 OR Session::get('rol') == 7) ? '' :  header('location: ' . URL . 'err/danger'); ?>
<?php
...
class ApiSunat
{
    ...
    public function sendDocSunaht($cod_ven,$num) {
        ...
        if ($documento->enviado_sunat == '1'):
            ...
            foreach ($resultado_detalle as $detalle ){
                ...
                // $total_bolsa += (Session::get('imp_val_bol') * $detalle['cantidad']);
                ...
                $productos[] = [
                    ...
                    // "POR_ICBPER" => Session::get('imp_val_bol'),
                    // "txtICBPER_DET" => Session::get('imp_val_bol') * $detalle['cantidad']
                ];
            }
            ...
            //CALCULOS INICIO
                if($total_ope_gravadas > 0){
                   ...
                   $total_gravadas = (($documento->total + $documento->comision_delivery) - $documento->descuento_monto) / igv_dec2;
                    ...
                    $sub_tot1 = number_format(((($documento->total + $documento->comision_delivery))), 2, '.', '');
                    $totales = number_format((($documento->total + $documento->comision_delivery) ), 2, '.', '');
                    $totales_2 = number_format((($documento->total + $documento->comision_delivery)- $documento->descuento_monto), 2, '.', '');
                    $total_gravadas = "0";
                    $total_exoneradas = ($documento->total + $documento->comision_delivery);
                    $dscto_global = number_format(($documento->descuento_monto), 2, '.', '');
                    $total_descuento = number_format(($documento->descuento_monto), 2, '.', '');
                }
            ...
            $data = [
                ...
                "total_bolsa" => 0, #subtotal
                // "total_bolsa" => $total_bolsa, #subtotal

                /*"porcentaje_igv" => "10.00", #El IGV debe ser 18, si lo tienes en igv_dec, conviértelo a 18*/
                ...
                "total" => $totales,
                "total_2" => $totales_2,
                "total_letras" => numtoletras($totales),
                "nro_guia_remision" => "",
                "cod_guia_remision" => "",
                "nro_otr_comprobante" => "",
                ...
                "emisor" => [
                    ...
                    "amazonas" => $empresa->amazonas,
                    "certpse" => $empresa->certpse
                ],
                //items del documento
                "detalle" => $productos,
                "ose" => [
            ...
            if(@$rs->respuesta == 'error' ){
                $enviado_sunat = 0;
                $code_respuesta_sunat = @$rs->cod_sunat;
                $descripcion_sunat_cdr = "".@$rs->cod_sunat."".@$rs->msj_sunat."";
                $name_file_sunat = @$rs->file;
                $hash_cdr = @$rs->hash_cdr;
                $hash_cpe = @$rs->hash_cpe;
    ...
    public function postComunicacionBaja($post) {
        ...
        $fecha = date('Y-m-d',strtotime($post['fecha']));//FECHA DEL COMPROBANTE
        ...
        if ($documento->estado == 'i'):
            ...
            $respuesta = array('enviado_sunat' => $enviado_sunat, 'mensaje' => $descripcion_sunat_cdr);
            // $respuesta = array('enviado_sunat' => $enviado_sunat, 'mensaje' => "falta");
